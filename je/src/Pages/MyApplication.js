...
import "../App.css";
import { Container, Navbar, NavDropdown, OverlayTrigger, Tooltip,ListGroup,Offcanvas,Modal,Button,FormControl, Pagination} from "react-bootstrap";
import logo from "../je-logo.jpg"
import { FaListAlt,FaGripHorizontal,FaShoppingCart,FaPeopleArrows,FaDatabase, FaEdit, FaPlus, FaArrowUp, FaArrowDown} from "react-icons/fa";
...
//   fetching data from API
    ...
    useEffect(()=>{
            Axios.get(`https://626118b3f429c20deb9a8ee7.mockapi.io/userData?page=${pageCount}&limit=8`).then(
            (response) =>{
    ...
    const sortDesc= () =>{
        Axios.get(`https://626118b3f429c20deb9a8ee7.mockapi.io/userData?sortBy=employeeName&order=desc`).then(
            (response) =>{
                setAPIdata(response.data)
            }
        )
    ...
    const sortAsc=() =>{
        Axios.get(`https://626118b3f429c20deb9a8ee7.mockapi.io/userData?sortBy=employeeName&order=asc?page=${pageCount}&limit=8`).then(
            (response) =>{
                setAPIdata(response.data)
            }
        )
    ...
    // searching fields in table

    const findRecord = (e) =>{
        setRecord(e.target.value)
        console.log(record)
        Axios.get(`https://626118b3f429c20deb9a8ee7.mockapi.io/userData?search=${record}`).then(
            (response) =>{
                setAPIdata(response.data)
            }
        )
                    ...
                    <div className="row" style={{height: "100vh"}}> 
                        ...
                        <div className="col-xl-10 col-md-12">
                            ...
                            <div className="row content-head  my-4 mx-3">
                                ...
                                <h3 className="fs-4 fw-bolder">My Application</h3>
                                ...
                                <div class="table-responsive p-0 m-0">
                                        <table class="table table-hover m-0 ">
                                            <thead className="thead w-100">
                                                <tr>
                                                    <th>Employee Name <FaArrowUp color="grey" onClick={()=>{sortAsc()}}/><FaArrowDown color="grey"  onClick={sortDesc}/></th>
                                                    <th>Cost Center</th>
                                                    <th>Expense Type</th>
                                                    <th><FormControl
                            ...
                            <div className="row">
                                <div className=" d-flex justify-content-center">
                                    <Pagination>
                                        <Pagination.Prev/>
                                        <Pagination.Item onClick={()=>{setPageCount(1)}}>{1}</Pagination.Item>
                                        <Pagination.Item onClick={()=>{setPageCount(2)}}>{2}</Pagination.Item>
                                        <Pagination.Next />
                                    </Pagination>
                                </div>
                            </div>
